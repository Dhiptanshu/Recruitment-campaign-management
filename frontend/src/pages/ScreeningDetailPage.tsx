              ...
              <span
                title={
                  s.ai_source === "llm"
                    ...
                    : "Generated by the deterministic fallback scorer (LLM unavailable or not configured)"
                }
                className={`rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${
                  s.ai_source === "llm" ? "bg-violet-100 text-violet-700" : "bg-slate-100 text-slate-500"
                }`}
              >
                {s.ai_source === "llm" ? "LLM" : "Heuristic fallback"}
              </span>
            )}
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-3 space-y-2">
            {s.transcript.map((turn, i) =>
              turn.speaker === "system" ? (
                <p key={i} className="text-center text-xs italic text-slate-400">{turn.text}</p>
              ) : (
                <div key={i} className={`flex ${turn.speaker === "candidate" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${
                      turn.speaker === "candidate" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-800"
                    }`}
                  >
                    <div className="mb-0.5 text-[10px] uppercase tracking-wide opacity-60">
                      {turn.speaker === "candidate" ? s.candidate.name.split(" ")[0] : "AI Agent"}
                    </div>
                    {turn.text}
                  </div>
                </div>
